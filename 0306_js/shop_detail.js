...
console.log(window.location.search); // 쿼리스트링 : window => 유사객체 구조 {key:value}
const urlParam = new URLSearchParams(window.location.search); // 쿼리스트링 추출, ?id=1&name=yujin&page=3
const productId = urlParam.get("id"); // id의 key:value만 추출
...
if (product) {
  productDetail.innerHTML = `<img src="${product.image}" alt="${product.name}" /><h2>${product.name}</h2><p>가격: ${product.price}원</p><p>${product.description}</p>`;
} else {
  productDetail.innerHTML = "<p>일치하는 상품이 없습니다!</p>";
}
